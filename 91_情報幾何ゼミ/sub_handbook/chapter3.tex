...
\begin{proposition}[$\wt{\Theta}$は凸集合]
...
\begin{example}[正規分布族]
    \label[example]{ex:normal-distribution}
    \TODO{$V$に修正}
    $\calX = \R$、
...
\begin{corollary}[自然パラメータの変換]
    \label[corollary]{cor:transformation-of-natural-parameters}
    上の定理の状況で、
...
\begin{corollary}
    \label[corollary]{cor:expectation-of-T}
...
\begin{proof}
    ...
    と同様の議論により補題の主張の等式が成り立つ\TODO{命題を一般化する}。
\end{proof}
...
\begin{proposition}[$\nabla^{(g)}, \nabla^{(\alpha)}$のACテンソルによる表示]
    \label[proposition]{prop:connections-by-AC-tensor}
    \TODO{テンソルの成分を${}_{ij}^k$から${}_{ij}^{\hphantom{ij}k}$の形に修正}
    $\calP$上の任意の$\nabla$-アファイン座標に関し、
    $\nabla^{(g)}$および$\nabla^{(\alpha)}$の
    接続係数は次をみたす:
    \begin{enumerate}
...
\begin{proposition}[捩率と曲率のACテンソルによる表示]
    \label[proposition]{prop:torsion-and-curvature-by-AC-tensor}
    $\calP$上の任意の$\nabla$-アファイン座標に関し、
...
\begin{proof}
    \cref{prop:components-of-AC-tensor}を用いると
    \begin{equation}
        S_{ijk}(p)
            = E_p [
                (T^i - E_p[T^i])
